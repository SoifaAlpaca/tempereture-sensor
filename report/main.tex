...
\section{Introduction}
    ...
    \label{requirements}
    For the system Design it's important to define the temperature interval in
which this system will work, thus it was define as $ T \in [10^\circ C; 40^\circ C]$. 
...
\subsection{NTC - Negative Temperature Coefficient}
    ...
    Temperature Coefficient (NTC), means that the sensor resistance decreases as temperature increases. This behavior exhibits a predictable response to temperature variations.

    o que ]e 
    info da datasheet 
    tirar o primeiro paragrafo de 3.1
...
\label{ AFENTC }
    ...
    Through the \textit{NTC}'s datasheet, according with the  \hyperref[requirements]{system requirements}, the interval of its resistance values is $R_{NTC} \in [~5,282k~;~19,98k~]$
    ...
    \textcolor{red}{DAR FIX AOS ABCS nao sei os valores}
    \begin{equation}
        \begin{cases}
...
\subsection{NTC}
    ...
    \textcolor{red}{Martim: Achei que se cahlhar separar entre esta sim e montecarlo }
    ...
    \label{MonteCarloNTC}
        ...
        A Montecarlo simulation is done in order to evaluate the AFE's output in relation to the normal distribution of the resistor values. Giving a sense of how reliable the circuit is.
        ...
        \textcolor{red}{Esta justificacao esta fraquinha}
        Since the output voltages surpass the defined range, the AFE gain should be lowered and/or the offset augmented. This was not done because this only happens in some cases and this way no resolution is lost. 
...
\subsection{LM35}
    ...
    \subsubsection{Montecarlo}
        For the same reasons explained in section \ref{MonteCarloNTC} a Montecarlo simulation is needed for this AFE.
