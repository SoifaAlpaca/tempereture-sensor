...
\usepackage{fancyhdr} % Head note and footnote
    ...
    \fancyhead[R]{\footnotesize Electronics for Micro-Systems} % Right of Head note
...
\subsection{Monte-Carlo Analysis} 
    ...
    The results for the NTC AFE are shown in figure \ref{montecarlontc}.
    ...
    And the results for the LM35 AFE circuits in figure \ref{montecarlolm}.
    ...
    By analysing the results of the Monte-Carlo simulation of the NTC AFE we can conclude that the
    ...
    For the results of the LM35 AFE Monte-Carlo test, the major difference between the thirty simulations is the slope of the output voltage,
    because the resistances in this circuit only affects its gain. This consequence is more problematic at higher temperatures, since the input voltage is greater at these values.

    To better visualise the variation in the output voltage of both systems at the maximum, minimum and average temperature, a more specific simulation was used, where the value of the $R_{NTC}$ and the $V_{LM35}$ were fixed for the temperature tested. So first of all, for 10ºC, the results are represented in figure \ref{10ºC - montecarlo}.
...
In order to improve the accuracy of the temperature readings, a multisampling technique was used. The ESP32 reads the output of the sensors multiple times and then calculates the average value. Because this acts as a low-pass filter, this technique helps to reduce noise and improve the accuracy of the temperature readings.
...
\label{future-work}
    ...
    In this section it will be discussed the possible improvements that could be made to the temperature sensing system to enhance its performance and accuracy.
...
\begin{itemize}
    \item \textbf{script.py} - Contains the code used to calculate the AFE circuits and the Monte-Carlo simulations.
    \item \textbf{main.cpp} - Contains the code used to program the ESP32.
    \item \textbf{GUI\_checkbox.py} - Contains the code used to develop the GUI.
    \item \textbf{GitHub repository} - \url{https://github.com/SoifaAlpaca/tempereture-sensor}
