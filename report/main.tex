...
\subsection{LM35 - Precision Centigrade Temperature Sensor}
    The LM35 is a precision integrated-circuit temperature sensor with an output voltage that is linearly proportional to the temperature in Celsius. Unlike other temperature sensors calibrated in Kelvin, the LM35 does not require the user to subtract a constant value to obtain Celsius scaling, simplifying the measurement process. The sensor offers a high level of accuracy, ensuring a typical error of just ±0.5°C at 25°C.
...
\subsection{Operational Amplifier}
    ...
    \textcolor{red}{TIVE DE BAZAR JA ACABO
    Se calhar meter esta seccao noutro sitio}
    For the following AFE's it's important to choose an appropriate Operational Amplifier (OpAmp). 

    The desired characteristics are:

    $\cdot$ Rail-to-Rail voltage.
    ...
    \hline
    \textbf{Characteristic}     & \textbf{MCP6001}               & \textbf{TL084}               & \textbf{Undefined Op-Amp} \\ \hline
    \textbf{Max Output (V)}     & VSS+25mV to VDD-25mV           & (V+)-1.5V                    & X.X                       \\ \hline
    \textbf{Supply Range (V)}   & 1.8 to 6.0                     & 4.5 to 40                    & X.X                       \\ \hline
    \textbf{Offset (mV)}        & ±4.5                           & 1                            & X.X                       \\ \hline
    \textbf{Max Output (mA)}    & ±6 mA (1.8V), ±23 mA (5.5V)    & N/A                          & X.X                       \\ \hline
    \textbf{Idle Consumption}   & 100 µA                         & 1.4 mA per channel           & X.X                       \\ \hline
    \end{tabular}
    \caption{Comparison of Key Parameters for MCP6001, TL084, and Undefined Op-Amp}
    \end{table}
