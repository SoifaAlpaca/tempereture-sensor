...
# Callback function for the relay control button


def toggle_relay():
    global relay_on
    relay_on = not relay_on
    relay_button.config(text="Turn Relay OFF" if relay_on else "Turn Relay ON")
    # Send command to MCU to toggle the relay (replace with actual command as needed)
    if relay_on:
...
def clear_graph():
    global ntc_values, lm_values, ds_values
    ntc_values.clear()
    lm_values.clear()
    ds_values.clear()
    ax.clear()
...
relay_on = False
relay_button = ttk.Button(root, text="Turn Relay ON", command=toggle_relay)
